...
DPSMate.Options.Options[1]["args"]["auraslost"] = {
	...
	get = function() return DPSMateSettings["windows"][(DPSMate.Options.Dewdrop:GetOpenedParent() or DPSMate).Key or 1]["options"][1]["auraslost"] end,
...
function DPSMate.Modules.AurasLost:GetSettingValues(arr, cbt, k)
	...
	for cat, val in pairs(sortedTable) do
		...
		local str = {[1]="",[2]="",[3]=""}
		if DPSMateSettings["columnsauraslost"][1] then str[1] = " "..DPSMate:Commas(dmg, k)..p; strt[2] = DPSMate:Commas(tot, k)..p end
