...
function DPSMate.Modules.DetailsDamage:UpdatePie(gg, cname)
	...
	for cat, val in pairs(uArr) do
		if (dArr[cat][2] and DPSMateSettings["mergepets"]) then user=DPSMateUser[DPSMateUser[cname or DetailsUser][5]][1] else user=DPSMateUser[cname or DetailsUser][1] end
...
function DPSMate.Modules.DetailsDamage:SortLineTable(t, b, cname)
	...
	if b then
		...
		-- Pet
		if DPSMateUser[cname or DetailsUser][5] and DPSMateUser[DPSMateUser[cname or DetailsUser][5]] then
